...
export class FanQieApi {
  ...
  public async searchBooks(key: string, tabType: number = 3, offset: number = 0): Promise<any> {
    console.error(`[FanQieApi] searchBooks: key=${key}, tab=${tabType}`);
    try {
      const url = '/api/search';
      console.error(`[FanQieApi] Requesting: ${this.baseUrl}${url} params=${JSON.stringify({ key, tab_type: tabType, offset })}`);
      const response = await this.axiosInstance.get(url, {
        params: { key, tab_type: tabType, offset }
      });
      console.error(`[FanQieApi] Response status: ${response.status}`);
      return this.handleResponse(response);
    } catch (error: any) {
      console.error(`[FanQieApi] Error: ${error.message}`, error.response?.data);
      throw new Error(`搜索书籍失败: ${error.message} (URL: ${this.baseUrl}/api/search)`);
    }
  }
