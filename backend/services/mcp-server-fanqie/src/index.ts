 ...
 * 简化搜索结果，减少网络传输量
 */
function simplifySearchResponse(data: any): any {
  if (!data || !data.search_tabs) return data;

  // 查找包含数据的tab
  const novelTab = data.search_tabs.find((t: any) => t.data && t.data.length > 0);
  if (!novelTab || !novelTab.data) {
    return { message: "未找到相关书籍", original_data: data };
  }

  const books = novelTab.data.map((item: any) => {
    ...
    return {
      ...
      author: book.author,
      abstract: book.abstract ? book.abstract.substring(0, 150) + (book.abstract.length > 150 ? "..." : "") : "",
      score: book.score,
      category: book.category,
      word_number: book.word_number,
      read_count: book.read_count,
      thumb_url: book.thumb_url,
      creation_status: book.creation_status === "1" ? "连载中" : "已完结"
    };
  }).filter((b: any) => b);
  ...
  try {
    ...
    switch (request.params.name) {
      ...
      case "search_books": {
        ...
        const simplifiedResult = simplifySearchResponse(result);
